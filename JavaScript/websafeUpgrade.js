...
function startingIntro(position) {
 ...
 introduction = setTimeout(secondIntroduction(position), 5000);
}
///4\\\
function secondIntroduction(position) {
 ...
 introduction = setTimeout(nextPartOfStory(position), 5000);
}
///5\\\Plays the next part of the story
function nextPartOfStory(position) {
 //5.1\\clears out the time if there was one and changes the image & top paragraph
 clearTimeout(introduction);
 fixImages(position);
 fixTopPar(position);
 introduction = setTimeout(nextPartOfStory(position), 10000);
 //5.2\\if i ='s any of these numbers
 switch (position) {
 case 0 || 2 || 3 || 4 || 5 || 11 || 12 || 15 || 16 || 19: 
   //5.21\\
   if (position == 0) {
    document.getElementById("backButton").style.visibility = "hidden";
    document.getElementById("middlepar").style.visibility = "hidden";
  ...
  case 7: 
    ...
    fixMiddlePar(position);
    fixBottomPar(position);
 position+= 1;
break;
 //5.5\\If it is any of these numbers execute 
  case 1 || 6 || 8 || 9 || 10 || 13 || 14 || 17 || 18: 
...
function fixTopPar(position) {
 ...
 document.getElementById("toppar").innerHTML = top[position];
}
///8\\\Middle paragraph function
function fixMiddlePar(position) {
 ...
 document.getElementById("middlepar").innerHTML = middle[position];
 }
///9\\\Is the bottom paragraph function
function fixBottomPar(position) {  
